/*
  ...
  BSCS 2-1
  Data structures
  EATMORE solution 1:2 different ordering distribution no queues 
  8/1/2019
  dev- c++
*/	
...
//***********************SOLUTION 2******************************************
Z=0;X=0;c1.pdone=0;c1.sdone=0;c1.hdone=0;c2.pdone=0;c2.sdone=0;c2.hdone=0;c3.pdone=0;c3.sdone=0;c3.hdone=0;c3.done=0;c1.time=0;c2.time=0;
PopcornAvailable=1;SodaAvailable=1;HotdogAvailable=1;X=0;c1.go=1;c2.go=1;c3.go=1;c1.done=0;c2.done=0;c3.time=0;

gotoxy(54,1);printf("POPCORN   SODA   HOTDOG ");
while(Z!=3){
if(PopcornAvailable==1){
	         if(c1.go==1&&c1.pdone==0&&PopcornAvailable==1){A=c1.popcorn;B=1;PopcornAvailable=0;c1.go=0;c1.pdone=1;}
		else if(c2.go==1&&c2.pdone==0&&PopcornAvailable==1){A=c2.popcorn;B=2;PopcornAvailable=0;c2.go=0;c2.pdone=1;}
		else if(c3.go==1&&c3.pdone==0&&PopcornAvailable==1){A=c3.popcorn;B=3;PopcornAvailable=0;c3.go=0;c3.pdone=1;}
	
		}
if(SodaAvailable==1){
	         if(c1.go==1&&c1.sdone==0&&SodaAvailable==1){C=c1.soda;D=1;SodaAvailable=0;c1.go=0;c1.sdone=1;}
		else if(c2.go==1&&c2.sdone==0&&SodaAvailable==1){C=c2.soda;D=2;SodaAvailable=0;c2.go=0;c2.sdone=1;}
		else if(c3.go==1&&c3.sdone==0&&SodaAvailable==1){C=c3.soda;D=3;SodaAvailable=0;c3.go=0;c3.sdone=1;}
...
else{gotoxy(57,2+X);printf("*");}
if(SodaAvailable==0){
	if(C!=0&&C>0){
		gotoxy(65,2+X);printf("%d",D);C--;
			if(C==0){
				SodaAvailable=1;
					     if(D==1)
						c1.go=1;
...
else{gotoxy(65,2+X);printf("*");}
if(HotdogAvailable==0){
	if(E!=0&&E>0){
		gotoxy(73,2+X);printf("%d",F);E--;
			if(E==0){
				HotdogAvailable=1;
					if(F==1)
						c1.go=1;
...
X++;
if(c1.done==1&&c2.done==1&&c3.done==1)Z=3;
gotoxy(52,1+X);printf("%d",X);
}c1.finish=X-(c1.time-1);c2.finish=X-(c2.time-1);c3.finish=X-(c3.time-1);
float Average2=float((c1.finish+c2.finish+c3.finish)/3);
gotoxy(58,3+X);printf("(2)Total Time: %d",X);
gotoxy(54,4+X);printf("---time spend buying----");
gotoxy(57,5+X);printf("Customer 1: %d min",c1.finish);
gotoxy(57,6+X);printf("Customer 2: %d min",c2.finish);
gotoxy(57,7+X);printf("Customer 3: %d min",c3.finish);
gotoxy(57,8+X);printf(">>>AVERAGE: %.0f min",Average2);
