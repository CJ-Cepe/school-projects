...
for(int j=0;j<10;j++){
	...
	int PP_Largest_Priority,PP_Largest_Location;
	int SRTF,PP,FCFS;
	...
	for(int i=0;i<number_of_process;i++){ // pang check ng level na gagawin
		...
		if(process[i].Level==1 && process[i].Arrival_Time <= Time && process[i].Status ==0){
			PP=1;
			if(process[i].Priority>PP_Largest_Priority)
			{
				...
				if(process[i].Level==0 && process[i].Arrival_Time <= Time && process[i].Status ==0){				
					if(process[i].Current_Burst_Time<Srtf_Smallest_Burst){
						...
						Srtf_Smallest_Location=i;	
							if(process[Current_Srtf].Current_Burst_Time == process[Srtf_Smallest_Location].Current_Burst_Time && Current_Srtf != Srtf_Smallest_Location ){
				...
				if(process[i].Level==1 && process[i].Arrival_Time <= Time && process[i].Status ==0){				
					if(process[i].Priority<PP_Smallest_Priority){
						...
						PP_Smallest_Location=i;	
							if(process[Current_PP].Priority == process[PP_Smallest_Location].Priority && Current_PP != PP_Smallest_Location ){
...
Time++;
	...
	for(int i=0;i<number_of_process;i++){
		printf("\n\t%c = %d - %d",process[i].Process_Id,process[i].Current_Burst_Time,process[i].Level);
	}
