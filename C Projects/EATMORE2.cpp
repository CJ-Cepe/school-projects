...
int TotalTime=1,A=0,B=0,C=0,J=0,K=0,L=0,T=0;
int M=0,N=0,O=0,P,Z=0,E=0,F=0,G=0,Q,R=0,S=0;
//***********PRINTTING***************
...
while(Z!=3){
if(Pavailable==1){
     if(C1.go==1&&C1.pdone==0&&Pavailable==1){A=C1.PopcornTime;Pavailable=0;C1.pdone=1;C1.go=0;for(P=0;P<A;P++){gotoxy(30,2+E);E++;printf("1");}}
else if(C2.go==1&&C2.pdone==0&&Pavailable==1){A=C2.PopcornTime;Pavailable=0;C2.pdone=1;C2.go=0;for(P=0;P<A;P++){gotoxy(30,2+E);E++;printf("2");}}
else if(C3.go==1&&C3.pdone==0&&Pavailable==1){A=C3.PopcornTime;Pavailable=0;C3.pdone=1;C3.go=0;for(P=0;P<A;P++){gotoxy(30,2+E);E++;printf("3");}}
else {gotoxy(30,2+E);printf("");E++;}
}
if(Savailable==1){
	 if(C1.go==1&&C1.sdone==0&&Savailable==1){B=C1.SodaTime;Savailable=0;C1.sdone=1;C1.go=0;for(P=0;P<B;P++){gotoxy(38,2+F);F++;printf("1");}}
else if(C2.go==1&&C2.sdone==0&&Savailable==1){B=C2.SodaTime;Savailable=0;C2.sdone=1;C2.go=0;for(P=0;P<B;P++){gotoxy(38,2+F);F++;printf("2");}}
else if(C3.go==1&&C3.sdone==0&&Savailable==1){B=C3.SodaTime;Savailable=0;C3.sdone=1;C3.go=0;for(P=0;P<B;P++){gotoxy(38,2+F);F++;printf("3");}}
else {gotoxy(38,2+F);printf("");F++;}
}
if(Havailable==1){
     if(C1.go==1&&C1.hdone==0&&Havailable==1){C=C1.HotdogTime;Havailable=0;C1.hdone=1;C1.go=0;for(P=0;P<C;P++){gotoxy(46,2+G);G++;printf("1");}}
else if(C2.go==1&&C2.hdone==0&&Havailable==1){C=C2.HotdogTime;Havailable=0;C2.hdone=1;C2.go=0;for(P=0;P<C;P++){gotoxy(46,2+G);G++;printf("2");}}
else if(C3.go==1&&C3.hdone==0&&Havailable==1){C=C3.HotdogTime;Havailable=0;C3.hdone=1;C3.go=0;for(P=0;P<C;P++){gotoxy(46,2+G);G++;printf("3");}}
...
if(B==0)C2.go=1;
if(C==0)C3.go=1;
if(C1.pdone==1&&C1.sdone==1&&C1.hdone==1){M=1;}
if(C2.pdone==1&&C2.sdone==1&&C2.hdone==1){N=1;}
if(C3.pdone==1&&C3.sdone==1&&C3.hdone==1){O=1;}
Z=M+N+O;
}//while
